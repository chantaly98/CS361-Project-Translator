...
def main():
    ...
    while True:
        time.sleep(1)
        with open('frontend/microservice/comm1.txt', 'r') as comm:
            if comm.read() == "first":
                open('frontend/microservice/comm1.txt', 'w').close()
                with open('frontend/microservice/CCA1.json', 'r') as request:
                    data = json.load(request)
                    if type(data) == str:
                        analysis = CCA(data)
                    open('frontend/microservice/CCA1.json', 'w').close()
                    with open('frontend/microservice/CCA1.json', 'w') as response:
                        json.dump(analysis, response)
        with open('frontend/microservice/comm2.txt', 'r') as comm:
            if comm.read() == "second":
                open('frontend/microservice/comm2.txt', 'w').close()
                with open('frontend/microservice/CCA2.json', 'r') as request:
                    data = json.load(request)
                    if type(data) == str:
                        analysis = CCA(data)
                    open('frontend/microservice/CCA2.json', 'w').close()
                    with open('frontend/microservice/CCA2.json', 'w') as response:
                        json.dump(analysis, response)
